<template>
  ...
  >
    <template #field :class="{ 'border-danger border': hasErrors }">
      <div
        class="attach-many-container"
        :class="{ 'border-danger border': hasErrors }"
      >
        <div
          v-if="currentField.showToolbar"
        ...
        >
          <div v-if="preview" class="flex justify-center p-3 w-full">
            <div class="text-xl">
              {{ __("Selected Items") }} ({{ selected.length }})
            </div>
          </div>
      ...
      >
        ...
        <span v-if="currentField.showPreview">
          <CheckboxWithLabel
            @input="togglePreview($event)"
            :checked="preview"
            class="cursor-pointer"
            >{{ __("Preview") }}</CheckboxWithLabel
          >
        </span>
...
export default {
  ...
  computed: {
    exclusiveItems() {
      // Always return an array, even if field or exclusiveItems is undefined
      if (this.field && Array.isArray(this.field.exclusiveItems)) {
    ...
    exclusiveSelected() {
      return this.selected.find((id) =>
        (Array.isArray(this.exclusiveItems) ? this.exclusiveItems : []).includes(id.toString())
      );
    },
  },
  ...
  methods: {
    isCheckboxDisabled(id) {
      ...
      const exclusiveItemsRaw = (this.field && this.field.meta && this.field.meta.exclusiveItems) ? this.field.meta.exclusiveItems : (Array.isArray(this.field.exclusiveItems) ? this.field.exclusiveItems : []);
      const exclusiveItems = Array.from(exclusiveItemsRaw).map(String);
      const isExclusive = exclusiveItems.includes(idStr);
      const selectedStr = selected.map(String);
      const selectedExclusives = selectedStr.filter(sel => exclusiveItems.includes(sel));
      const anyExclusiveSelected = selectedExclusives.length > 0;
      if (anyExclusiveSelected) {
        return !selectedExclusives.includes(idStr);
      }
      if (selected.length > 0 && !anyExclusiveSelected && isExclusive) {
        return true;
      }
      return false;
    },
    ...
    toggle(event, id) {
      ...
      if (selectedStr.includes(idStr)) {
        this.selected = selected.filter(selectedId => selectedId.toString() !== idStr);
        return;
      }
      if (isExclusive) {
        this.selected = [idStr];
    ...
    retrieveData(keepSelected = false) {
      ...
      if (this.resourceId) {
        ...
        Nova.request(
          baseUrl + this.resourceName + "/attachable/" + this.field.attribute
        ).then((data) => {
          this.available = data.data.available || [];
          this.loading = false;
        });
