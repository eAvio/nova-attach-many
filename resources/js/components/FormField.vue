          ...
          >
            <template v-if="isCheckboxDisabled(resource.value)">
              <span class="text-xs text-gray-400 ml-2">{{
                exclusiveSelected
                  ? __("Exclusive selection active")
                  : __("Cannot combine with exclusive")
              }}</span>
            </template>
...
export default {
  ...
  computed: {
    ...
    exclusiveSelected() {
      return this.selected.find((id) =>
        this.exclusiveItems.includes(id.toString())
      );
    },
  },
  ...
  methods: {
    isCheckboxDisabled(id) {
      ...
      const anyExclusive = this.selected.some((sel) =>
        this.exclusiveItems.includes(sel.toString())
      );
      const isExclusive = this.exclusiveItems.includes(idStr);
      if (anyExclusive) {
        const disabled = !this.selected.includes(idStr);
        if (disabled) {
          console.log(
            "[AttachMany] Checkbox for",
            idStr,
            "disabled because an exclusive item is selected:",
            this.selected
          );
        }
        ...
        console.log(
          "[AttachMany] Checkbox for exclusive item",
          idStr,
          "disabled because other items are selected:",
          this.selected
        );
    ...
    toggle(event, id) {
      ...
      if (this.selected.map((sel) => sel.toString()).includes(idStr)) {
        ...
        this.selected = this.selected.filter(
          (selectedId) => selectedId.toString() != idStr
        );
      } else if (isExclusive) {
        console.log(
          "[AttachMany] Selecting exclusive item",
          idStr,
        ...
        console.log(
          "[AttachMany] Prevented selection of",
          idStr,
          "because an exclusive item is already selected:",
          this.selected
        );
